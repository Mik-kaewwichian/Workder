...
let OAuthService = class OAuthService {
    ...
    async handleGoogleOAuth(idToken, role) {
        ...
        return this.findOrCreateUserAndIssueToken(email, (tokenInfo.given_name || '').trim(), (tokenInfo.family_name || '').trim(), role);
    }
    async handleFacebookOAuth(accessToken, expectedUserId, role) {
        const profile = await this.verifyFacebookAccessToken(accessToken, expectedUserId);
        const email = profile.email?.trim().toLowerCase();
        if (!email) {
            throw new common_1.BadRequestException('Facebook account does not provide email');
        }
        return this.findOrCreateUserAndIssueToken(email, (profile.first_name || '').trim(), (profile.last_name || '').trim(), role);
    }
