...
import ProofPhotoModal from './ProofPhotoModal';
                                        ...
                                        <div key={e.id} className="px-4 py-3 space-y-2">
                                            ...
                                            {e.status === 'HELD' && (
                                                <>
                                                    ...
                                                    <button
                                                        disabled={busy}
                                                        onClick={() => setProofModal({ escrowId: e.id, jobTitle: e.job.title })}
                        ...
                        {asEmployer.length > 0 && (
                            <div>
                                ...
                                {asEmployer.map((e) => {
                                    ...
                                    // Parse proof photos if available
                                    let proofUrls: string[] = [];
                                    if (e.proofPhotos) {
                                        try { proofUrls = JSON.parse(e.proofPhotos); } catch { /* ignore */ }
                                    }
                                                    ...
                                                    {proofUrls.length > 0 && (
                                                        <div>
                                                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">
                                                                หลักฐานจาก Worker ({proofUrls.length} รูป)
                                                            </p>
                                                            <div className="flex gap-1.5 flex-wrap">
                                                                {proofUrls.map((url, i) => (
                                                                    <a key={i} href={url} target="_blank" rel="noreferrer" className="block w-14 h-14 rounded-lg overflow-hidden border border-amber-200 hover:opacity-80 transition-opacity">
